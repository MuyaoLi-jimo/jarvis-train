...
version="mc-llava_v1.6_mistral_7b-lora-11-10-craft-craft_table-shell_agent-hard-mistral-11-10-2"  # {model_version}_{dataset_version}_{training_date}"
...
export WANDB_NOTES="[24-11-09] 55.3k vision language convs convs (instruction state action),with image augment"

deepspeed --include localhost:$cuda_visible_devices --master_port=$training_port ultron/model/train/vsft.py \
    --dataset_name $dataset_name \
    --model_name_or_path "/scratch/models/llava-v1.6-mistral-7b-hf" \
    --report_to "wandb" \
